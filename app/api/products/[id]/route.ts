...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabase.from('crm_products').select('*').eq('id', id).eq('org_id', orgId).single()
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error: dbErr } = await supabase.from('crm_products')
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { orgId, id: actorId } = session.user
  const { id } = await params
  const { error: dbErr } = await supabase.from('crm_products').delete().eq('id', id).eq('org_id', orgId)
  if (dbErr) return NextResponse.json({ error: dbErr.message }, { status: 500 })
