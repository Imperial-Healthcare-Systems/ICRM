...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data } = await supabase.from('crm_territories')
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data, error: dbErr } = await supabase.from('crm_territories')
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!['super_admin', 'admin'].includes(role)) return NextResponse.json({ error: 'Insufficient permissions.' }, { status: 403 })
  const { id } = await params
  const { error: dbErr } = await supabase.from('crm_territories').delete().eq('id', id).eq('org_id', orgId)
  if (dbErr) return NextResponse.json({ error: dbErr.message }, { status: 500 })
